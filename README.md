...
From the two tables, we can see that whether it is the validation set under normal conditions or the validation set under chaotic conditions, the fine-tuning model with data augmentation through EDA maintains higher accuracy in most models than the fine-tuning model without data augmentation. Among the models with better performance, the fine-tuning model with data augmentation has an accuracy improvement of 2.41% compared to the fine-tuning model without data augmentation. Proved the feasibility of the method.
And our method also has better stability. In chaotic data, our model enhanced with EDA data has shown good performance in most models, with accuracies of 77.17% (LLaMA-3-8B), 78.38% (Bloomz-7B1), and 77.21% (Yi-6B), respectively. The accuracy is basically similar to that of the normal validation set, with a difference of 1.89% in accuracy, which can guarantee corresponding stability. However, in Mistral-7B-Instruct-v0.2, the fine-tuning model after data augmentation using EDA did not exceed the accuracy of the fine-tuning model without selected data augmentation. This may have been due to overfitting, but in most models, our method has produced excellent results.
From the perspective of both normal and chaotic validation sets, Bloomz-7B1 has the best accuracy in sentiment analysis in the field of air conditioning assembly, with an average accuracy of 79.26%, higher than LLaMA-3-8B and Yi-6B's 78.17%. In terms of validation time and efficiency, Bloomz-7B1 has less delay compared to LLaMA-3-8B and Yi-6B, and can quickly respond and process tasks, avoiding some delay phenomena and corresponding output problems. Therefore, in the air conditioning assembly instance, we chose the Bloomz-7B1 base model for fine-tuning, and the fine-tuning dataset is the relevant user demand recognition data enhanced by EDA data.
